...
export default function Header() {
  ...
  const algoPrice = prices['algorand']?.usd || 0;
  const algo24hChange = prices['algorand']?.usd_24h_change || 0;
      ...
      >
        ...
        <Box display={{ base: 'none', md: 'block' }} mr={4}>
          <Tooltip label="ALGO Price">
            <Tag 
              size="md" 
              colorScheme={algo24hChange >= 0 ? 'green' : 'red'} 
              borderRadius="full" 
              variant="solid" 
              className="glow"
            >
              <HStack spacing={1}>
                ...
                <Text fontWeight="bold">${algoPrice.toFixed(2)}</Text>
                <Box>
                  <Icon 
                    as={algo24hChange >= 0 ? ArrowUpIcon : ArrowDownIcon} 
                    boxSize={3} 
                  />
                  <Text fontSize="xs" as="span">
                    {Math.abs(algo24hChange).toFixed(2)}%
                  </Text>
                </Box>
              </HStack>
            </Tag>
              ...
              <HStack spacing={3} display={{ base: 'none', md: 'flex' }}>
                ...
                <Tooltip label="ALGO Balance">
                  <Tag colorScheme="purple" borderRadius="full" px={3} py={2}>
                    <HStack>
                      <Icon as={SiAlgorand} color="purple.500" />
